...
|long long|8|8|LL$|
|float|6位有效|4(7位有效)|-|
|double|10位有效|8(16位有效)|F$|
|long double|10位有效|16|L$|

- 定义一些确定长度的类型保证移植性如int64_t等, 可用U最为后缀指定无符号类型
...
- const 对象必须初始化,且之后被赋值的对象不能再改变 如const int a=10; a=11; 错误 
- cosnt 对象在编译过程中会被替换成对应值(意味着编译期就得到计算结果),只在文件内有效
```c++
//a.cpp
#include<iostream>
const int a=3;
...
- const int &r1=var; 其中var可以是表达式结果,非常量对象,字面值 ,不同类型对象
...
```c++
    ...
    double a=2.3; const int &b=c; // &a: 0x60fe84 &b: 0x60fe78 产生了新临时量, 地址不同
```

#### 指针与const
- 指向常量的指针不能通过指针改变所指对象的值`const int *ptr=&const_var; *ptr=10; //错误`
- 指向常量的指针可以指向不是常量的同类对象(与一般指针所指对象类型相同不同之处)
- 常量指针 `int *const ptr=&var; *ptr=10; // 正确`  指针是一个const对象,必须初始化且无法改变内容,且必须类型一致,但所指内容可以更改
- 顶层const 指指针(和其他任意对象)本身为常量 , 底层const 指指针(引用)所指对象为常量
...
int main(){
  ...
  const int *ptr_to_const;    //底层const 可以不初始化
  int *const const_ptr=&var;  //顶层const 作为常量必须初始化
...
#### 常量表达式 
- 常量表达式指编译过程中可以得到计算结果的表达式,一般由初始值和数据类型决定(const对象也是常量表达式)
- 可以被constexpr限定的类型叫做字面值类型(算术类型,引用,指针,自定义的字面值常量类和枚举类型)
- 对于算术类型来说,`constexpr int a=b;` 其中b一定是一个常量或者字面值常量
- 对于引用来说 `constexpr int &a=b;` 其中b是一个静态内存中定义的对象(可以不是常量)
- 对于指针来说 `constexpr int *a=&b;` a是一个常量指针 其中b是一个静态内存中定义的对象,可以不是常量或者为nullptr
- 对于指针来说 `constexpr const int *a=&b;` a是一个指向常量的常量指针 其中b是一个在静态内存中定义的对象

#### 关于c++内存区分类 
- 栈区,由编译器自动分配释放,存放函数的参数值,局部变量的值等
- 堆区,由程序员分配释放, 若程序员不释放,程序结束时可能由OS回收
- 全局区(静态区),全局变量和静态变量的存储是放在一块,未初始化与初始化的分开放置
...
const T2 *a2;  // a2是一个指针, 指向 指向int的常量指针
...
int a=10, &b=a; auto c=b; // 以引用所指类型为准 a是int型
...
int a1[10][2]={};    
auto a3=a1;  // int (*)[2] 类型        
auto a2=&a1; // int (*)[10][2] 类型    
    
int b1[10][10][2]={};    
auto b3=b1;  // int (*)[10][2] 类型    
auto b2=&b1; // int (*)[10][10][2]类型    
```

#### decltype类型指示符
```c++
int a=0, &b=a; decltype(b) c=a; // 返回相同的类型 c是int&
/* 表达式或对象作为表达式返回结果可以作为左值则返回结果类型的引用*/
...
2. `#ifndef FILENAME_H #define FILENAME_H ... #endif` 通过头文件保护符来避免重复, 注意一定要唯一
3. `#pragma once` 让编译器提供保证, 同一个文件物理上不会被包含多次


3. (2.38) 比较auto 与 decltype 的区别
	1. auto 忽略顶层const,忽略& 而 decltype 不忽略
	3. decltype 可以用 () , 表达式结果为左值 , \*ptr 来获得相应的引用类型
	4. auto保留数组元素指针(除非用 auto it = &array 指向数组 或 auto &it=array),decltype()保留数组类型
