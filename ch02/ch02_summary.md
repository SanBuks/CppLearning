...
- 无长度限制, 大小写敏感, 存在保留关键字
- 函数体内不能以 \_\_ 或\_[A-Z] 开头, 函数体外标识符不能以 \_ 开头(标准库保留了一些名字)
- 代码规范参考<<Google C++ Style Guide>>
...
- const int &r1=var; 其中var可以是 可转换为int的表达式结果, 非常量对象或字面值
...
- 静态全局变量(包括类与函数静态数据成员)则根据类型和初始化值分类 :
  - 先进行静态初始化后进行动态初始化
  - 静态初始化(编译链接过程中) : 
    - 如果是内置类型 : 有初识值则const init, 没有初识值 则zero init 
...
auto &a4=a1; // a4 防止退化 仍为 int [10][2] 类型
```
