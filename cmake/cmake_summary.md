...
#   1.1) 必须最开始调用且不要放在函数中
#   1.2) <min> 和 <policy_max> 版本格式为 major.minor[.patch[.tweak]]
#   1.3) 设定变量 ${CMAKE_MINIMUM_REQUIRED_VERSION} 为版本号
#   1.4) 隐式调用了 cmake_policy(VERSION <min>[...<max>])
cmake_minimum_required(VERSION 3.20)
# 2. cmake_policy(VERSION <min>[...<max>])
#   2.1) 每个版本引入若干个 CMP<NNNN>, 有 OLD 和 NEW 两种策略, 默认为 NEW 策略
#   2.2) max 版本可以超过当前 cmake 版本, 所有范围内
#   3.3) CMAKE_POLICY_DEFAULT_CMP<NNNN> 
# 3. 设置 项目名称 和 项目版本号 (优先设置)
project(Tutorial VERSION 1.0)
# 4. 增加 可执行目标 手动命名/用预设变量
