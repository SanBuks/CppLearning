...
# 2.27 底层 const 和 顶层 const
> 下面的哪些初始化是合法的？请说明原因。
```c++
int i = -1, &r = 0;         // 非法, 引用r需绑定到一个左值
int *const p2 = &i2;        // i2 是 const int 非法, 只是 int 合法
...
int main(){
	...
	c=42; //*正确, c : int, 以引用绑定类型为准, 引用绑定为 const int, 乎略顶层const
