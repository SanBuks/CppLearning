...
|类型|最小字节数/最小有效位数|实际大小(Linux64)|字符/字面值常量前后缀|
| --- | --- | --- | --- |
|bool|未定义|1|默认|
|char|1|1|^u8 (UTF-8)|
...
- LLP(Win64) 规范下: int - 4B; long - 4B; wchar_t -4B
- LP(Unix64) 规范下: int - 4B; long - 8B; wchar_t -8B
## 1.2 类型转换
...
```c++
// 用户自定以标识符的限制
int _a = 3;     // error  函数体外 不能以 下划线开头
int main() {
  int __A = 0;  // error  不能以 两个下划线 开头
  int _A = 1;   // error  不能以 一个下划线 + 大写字母开头 开头
  int _b = 2;   // accept 一个下划线 + 小写字母 开头
  return 0;
}
```
...
- 指针的值有: 
  ...
  - nullptr/0/NULL 
  - 无效 
- NULL: 定义在 cstdlib 为宏, 值为 0, 尽量用 nullptr
## 3.3 复合类型声明
...
- 顶层 const 被拷贝时无影响, 底层 const 被拷贝时需要所赋值的对象所指为常量 (底层 const 相同) 
```c++
int main() {
  int var = 0;                                 // 普通变量
...
- 文字常量区: 常量字符串存放区(可见字符串字面值常量是一个左值), 程序结束后由系统释放
- 程序代码区: 存放函数体的二进制代码
```c++
int a = 0;                  // 全局区 已初始化
char *p1;                   // 全局区 未初始化
...
int main() {                
  ...
  strcpy(p1, "123456");     // "123456\0" 放在常量区, 编译器可将它与 p3 所指向的 "123456" 优化成一个地方
  return 0;
} 
```
...
### GBK 字符集
- 两字节大小, 同时也是编码方式
- 前者为拓展字符后者为任意字符来年再一块为汉字(宽字符), 其余为ASCII字符
- 高字节为拓展字符, 低字节为任意编码
### UCS 字符集
- UCS 2: U+0000～U+FFFF
