...
auto &istream = std::getline(std::cin, s1, '\t');  // 读入一行包括 '\t', 舍去 '\t' 后赋予 s1 内容, 返回 is

auto &istream = cin.getline(pstr, 30);             // 同上, 赋予 pstr 指向的字符数组, 最大字符个数为 30
...
- for-range 语句或其他循环遍历语句的循环体中不该改变容器大小
...
// 二分查找, 找到所指迭代器
// 或者可以有序插入的位置, 如果是首前位置则指向首位置
template<typename T>
It<T> find(const std::vector<T> &vec, const T &e) {
  auto lo = vec.begin();     // 获取首迭代器 [begin,
  auto hi = vec.end();       // 获取尾后迭代器 end)
  while (lo != hi) {         // 判断迭代器是否指向同一个位置
    auto mid = lo + (hi - lo) / 2;
    if (e < *mid) {
      hi = mid;
    } else {
...
- 数组存在退化, 无法被拷贝和赋值, 但可用于 for-range 语句
```c++
...
```c++
#include <iterator>
int a[10];                   // 内置数组的下标是带符号类型, 区别于 vector
auto bit = std::begin(a);    // 获取首元素指针
auto eit = std::end(a);      // 获取尾后指针

ptrdiff_t diff = eit - bit;  // 指针相减类型为 ptrdiff_t, 空指针相减为0
...
// 2. 相关函数 (strcmp, strlen 略)
//  1) char *strncpy(char *dest, const char *src, std::size_t count);
//     将 src 复制到 dest, 最多复制 count 个字符, 可以包括'\0', 存在把 容量不够将'\0'覆盖的情况 
...
// 3. string 转换成字符数组指针 (注意 const 防止 string 发生变化, 最好拷贝一份)
