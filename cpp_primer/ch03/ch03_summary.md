...
std::string s1;                           // 默认初始化
std::string s2("123");                    // 直接拷贝初始化
std::string s3(s2);                       // 直接拷贝初始化
std::string s4 = "123";                   // 赋值拷贝初始化
std::string s5(2, 'c');                   // 直接初始化
                                          
std::cint >> s1;                          // 读入 s1, 忽略空白符读入, 以空白符分隔
std::cout << s2;                          // 打印 s2
                                          
assert(!s1.empty() && s1.size() > 1);     // 返回是否为空, 返回字符个数 (string 不包含 '\0')
std::string::size_type size = s1.size();  // 字符个数大小
                                          
assert(s1 != s2 || s3 < s4);              // 按字典序再按个数比较
                                          
s1[0] = 'x';                              // 下标返回引用
s1 = s2 + s3;                             // 忽略 `\0` 连接两个字符串, 返回新对象
auto a = "123"+"234";                     // error, 不能将两个字面值常量直接相加
```

## 2.2 geline 函数
```c++
auto &istream = std::getline(std::cin, s1);        // 读入一行包括 \n, 舍去后赋予 s1 内容, 返回is
auto &istream = std::getline(std::cin, s1, '\t');  // 读入一行包括 \t, 舍去后赋予 s1 内容, 返回is

auto &istream = cin.getline(pstr, 30);             // 同上, 赋予 pstr 指向的字符数组
auto &istream = cin.getline(pstr, 30, '\n');       // 同上, 赋予 pstr 指向的字符数组 
```
...
std::vector<string> a{10};        // 先按列表初始化 如果无法执行则值初始化
...
- for-range 语句 或 其他循环遍历语句 内不应该改变容器大小
...
// 3. string转换成字符数组指针 注意是 const 防止 string 发生变化, 最好拷贝一份
...
// 4. 相关函数
// 1) char *strncpy(char *dest, const char *src, std::size_t count);
// 将 src 复制到 dest, 最多复制 count 个字符(可以包括'\0'), 当 src 的长度小于 n 时, dest 的剩余部分将用空字节填充
char dest[] = "***\0*******";
char src[] = "123";
strncpy(dest, src, 8);  // 123\0\0\0\0\0***\0

// 2) char *strncat(char *dest, const char *src, std::size_t count);
// 将 src 复制到 dest, 最多复制 count 个字符并额外包含'\0', 直到到达 '\0' 或 字数满足 n 为止
char dest[] = "***\0*******";
char src[] = "123";
