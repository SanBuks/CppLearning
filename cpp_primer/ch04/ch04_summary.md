...
- 右值: 用对象的值 (如用于赋值的对象, 取地址的结果, 只用于赋值的临时量)
- 通用原则: 左值可以当作右值使用, 反过来需要特殊转换
- 操作符与返回值: 不同操作符作用于对象后的结果不同, 还会影响 decltype 类型推断的结果
```c++
int a = 3;
int left = a;            // a  作为结果为右值, 可看作发生了左值->右值转换
int *p = &a;             // &a 作为结果为右值
                         // p  作为结果为左值
...
void a(T &); void b(T &); void c(T &);
return a(my_io) + b(my_io) - c(my_io);
```
...
## 2.4 递增递减运算符
- 前置: 运算后作为求值结果(左值), 优先级与解引用相同, 右结合
- 后置: 运算后以改变之前保留的副本作为求值结果(右值), 优先级小于成员选择, 大于解引用, 右结合
- 尽量用前置递增递减, 尤其运用在复杂类型上, 可以减少复杂类型的额外存储
```c++
*p++;              // 等价
*(p++);

// 递增对象出现在同一表达式的两个地方容易发生未定义问题
*beg = f(*beg++);  // 未定义
```
...
- 左移右侧增加 0, 正数右移左侧增加 0, 负数右移则根据实现而定(一般增加的是符号位)
...
## 2.8 其他
- 条件运算符(三目运算符)优先级较低, 一般要加括号, 满足右结合律
...
### 算术类型转化过程
- 在计算表达式之前执行整型提升
- 如果类型不同符号相同, 则小类型转换为大类型
- 如果类型和符号不同, 则根据类型大小进行无符号类型转换 或 小类型转换成大类型
...
auto r2 = a + c;  // 这里 long 为 64 位, int 为 32 位, 则 unsigned int => long
```
...
// 在大部分表达式中, 数组名退化成指针
int *p = array;
// 在大部分表达式中, 函数名也退化成指针, 也具有以下类似的转换
auto p_func = func_name;

// 不发生退化的情况 (还有 typeid)
decltype(array) s = {};              // decltype
std::cout << sizeof array << "\n";  // sizeof
int (&array_ref)[2] = array;         // 数组的引用
std::cout << array << "\n"           // 0xc0a7dffd68 指向数组开始
          << array + 1 << "\n"       // 0xc0a7dffd6c 指向第二个元素
          << &array + 1 << "\n";     // 0xc0a7dffd70 指向数组末尾
```
